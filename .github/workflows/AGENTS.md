...
## Files
- `ci.yml` — workflow `CI`. Triggers on `push` and `pull_request` to `main`. Three independent parallel jobs (no `needs`), all `runs-on: ubuntu-latest`, all using `actions/checkout@v5` + `actions/setup-node@v5` (Node 24, `cache: npm`) then `npm ci`:
...
- `security.yml` — workflow `Security`. Triggers on `push`/`pull_request` to `main` **and** a weekly `schedule` (`cron: '0 6 * * 1'` = Mondays 06:00 UTC). Top-level `permissions: security-events: write, contents: read`. Three jobs:
  - `dependency-audit` — Node 24 (`cache: npm`), `npm ci`, then `npm audit --audit-level=high` (`continue-on-error: true`, report-only) and `npm audit --audit-level=critical` (fails the job on critical vulns only).
  - `trivy` — `aquasecurity/trivy-action@master` filesystem scan (`scan-type: fs`, `scan-ref: '.'`) emitting `trivy-results.sarif`, uploaded via `github/codeql-action/upload-sarif@v4` with `if: always()`.
  - `codeql` — job-level `security-events: write`; `github/codeql-action/init@v4` (`languages: javascript-typescript`) then `analyze@v4`.

## Conventions & best practices
- Pin actions the way the existing files do: `actions/*` are pinned to `@v5` and `github/codeql-action/*` to `@v4`; only `aquasecurity/trivy-action` uses `@master`. Match the surrounding style when adding steps.
- Node is `24` everywhere. Keep it consistent across jobs and both files.
...
2. Start with `- uses: actions/checkout@v5` then `- uses: actions/setup-node@v5` with `node-version: 24` and `cache: npm`.
...
5. If it produces SARIF, upload it with `github/codeql-action/upload-sarif@v4` and `if: always()`, and confirm `security-events: write` is granted (top-level or job-level).
